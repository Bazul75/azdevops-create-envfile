...
async function run() {
    try {
        ...
        const lines = Object.entries(group.variables as Record<string, { value: string }>)
            .map(([key]) => {
                const value = tl.getVariable(key);
                if (value === undefined) {
                    throw new Error(`The variables "${key}" is undefined. Make sure the Variable Group ${groupName} is linked/imported in your pipeline YAML or Pipeline Clasic.`);
                }
                return `${key}="${tl.getVariable(key)}"`;
            });
